/**
 * Campaign Analytics Sync API Route
 * Story 10.3: Instantly Analytics Service (Polling)
 *
 * POST /api/campaigns/[campaignId]/analytics/sync — Trigger manual sync
 * AC: #2 — SyncResult with analytics + daily analytics + lastSyncAt
...
export async function POST(_request: Request, { params }: RouteParams) {
  try {
    ...
    const syncResult = await service.syncAnalytics({
      apiKey,
      externalCampaignId: campaign.external_campaign_id,
    });

    // Use local campaignId
    return NextResponse.json({
      data: {
        ...syncResult,
        campaignId,
        analytics: { ...syncResult.analytics, campaignId },
      },
    });
  } catch (error) {
    ...
    return NextResponse.json(
      { error: "Erro interno ao sincronizar analytics" },
      { status: 500 }
    );
  }
}
